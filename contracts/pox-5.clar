...
;; ============================================================================
;; TEST/COMPILE SHIM (v1 - clarity-pox-5-pool)
;; The WIP native `get-bitcoin-tx-output?` used by `validate-l1-lockup` is not yet
;; present in released clarity-vm / clarinet. The L1 BTC-lockup path is NOT
;; exercised by the sBTC-only v1 pool, so this typed stub stands in only so the
;; vendored contract compiles for local testing. It returns the lockup's own
;; amount/expected-script, making the L1 verifier a no-op.
;; MUST be removed (and the real `get-bitcoin-tx-output?` call restored) before
;; this contract is used for anything touching the L1 lockup path.
(define-private (get-bitcoin-tx-output-shim
    ...
    (begin
        ;; pins the response error type to uint so callers' `try!` type-checks
        (asserts! true (err u0))
        (ok {
...
;; TEST/COMPILE SHIM (v1) - the WIP native `verify-merkle-proof` is likewise not in
;; released clarity-vm. Same rationale and caveat as `get-bitcoin-tx-output-shim`.
(define-private (verify-merkle-proof-shim
        ;; #[allow(unused_binding)]
...
(define-private (validate-l1-lockup
    ...
    (let (
            ...
            (output (try! (get-bitcoin-tx-output-shim (get tx lockup)
                (get output-index lockup) (get amount lockup)
                (get expected-script-hash accumulator)
            )))
        ...
        (asserts!
            (or
                ...
                (verify-merkle-proof-shim reversed-txid
