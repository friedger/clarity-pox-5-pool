...
(define-public (claim-rewards
    ...
    (let ((result (try! (contract-call? 'SP000000000000000000002Q6VF78.pox-5 claim-rewards bond-periods reward-cycle))))
...
(define-read-only (get-earned-staker-rewards
    ...
    (let (
            (earned-before-fees (contract-call? 'SP000000000000000000002Q6VF78.pox-5 get-earned-staker-rewards current-contract
...
(define-public (claim-staker-rewards
    ...
    (let (
            ...
            (rewards-info (unwrap-panic (contract-call? 'SP000000000000000000002Q6VF78.pox-5 claim-staker-rewards-for-signer staker
...
(define-public (register-self
    ...
    (begin
        (try! (authorize-admin))
        (try! (contract-call? 'SP000000000000000000002Q6VF78.pox-5 grant-signer-key signer-key current-contract
            auth-id signer-sig
        ))
        (contract-call? 'SP000000000000000000002Q6VF78.pox-5 register-signer signer-manager signer-key)
    )
)
...
(define-private (authorize-pox-5)
    (ok (asserts! (is-eq contract-caller 'SP000000000000000000002Q6VF78.pox-5) ERR_UNAUTHORIZED_CALLER))
)
